...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" 
        ...
        queryBinding="xslt2">
	<title>Schematron Prescrizione Farmaceutica No SSN 1.0</title>
	...
	<rule context="hl7:ClinicalDocument/hl7:author">
		...
		<!--rule context="//*[contains(local-name(),'Organization')][hl7:addr]">
			<assert test="not(hl7:addr[@use='H'] or hl7:addr[@use='HP'] or hl7:addr[@use='HV'])"
			>ERRORE-66| L'elemento 'addr' di un'organizzazione non può essere di tipo Home, ovvero l'attributo @use deve essere diverso da: H | HP | HV.
			</assert>
		</rule-->
		
		<rule context="//*[contains(local-name(),'Organization')][hl7:addr]">
			<assert test="count(not(hl7:addr[@use='H' or @use='HP' or @use='HV']))=1"
			>ERRORE-66| L'elemento 'addr' di un'organizzazione non può essere di tipo Home, ovvero l'attributo @use deve essere diverso da: H | HP | HV.
			</assert>
		</rule>
	...
	<rule context="hl7:ClinicalDocument/hl7:component/hl7:structuredBody">
		...
		<rule context="hl7:ClinicalDocument/hl7:component/hl7:structuredBody/hl7:component/hl7:section[hl7:code[@code='57828-6']]/hl7:entry">
			...
			>ERRORE-b34| Sezione Prescrizioni: L'elemento entry/substanceAdministration deve contenere un elemento effectiveTime che codifica l'ora di inizio e fine della terapia farmacologica o la sua durata. Se non sono disponibili informazioni per il periodo di dosaggio, deve essere fornito un attributo @nullFlavor con il valore 'UNK'.</assert>
			
			<assert test="count(hl7:substanceAdministration/hl7:effectiveTime[not(@xsi:type='IVL_TS' or @xsi:type='PIVL_TS' or @xsi:type='EIVL_TS' or @xsi:type='SXPR_TS')])=1"
			>ERRORE-b35| Sezione Prescrizioni: L'elemento entry/substanceAdministration può contenere un elemento effectiveTime di tipo TS, PIVL_TS, EIVL_TS  </assert>
			...
			<let name="subEntry" value="hl7:substanceAdministration/hl7:entryRelationship[@typeCode='SUBJ' and @inversionInd ='true']"/>
			...
			<assert test="count($subEntry)=0 or 
			count($subEntry/hl7:observation[hl7:templateId[@root='2.16.840.1.113883.3.1937.777.63.10.795']]/hl7:code[@code='N' and @codeSystem='2.16.840.1.113883.3.1937.777.63.11.26'])=1"
			...
			<assert test="count($subEntry)=0 or 
			count($subEntry/hl7:observation[hl7:templateId[@root='2.16.840.1.113883.3.1937.777.63.10.795']]/hl7:entryRelationship[@typeCode='RSON'])>=1"
